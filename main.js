...
const globe = document.querySelector('.globe'),
    slides = document.querySelectorAll('.earth'),
    poles = document.querySelector('.poles'),
    canvas = document.getElementsByTagName('canvas'),
    north = document.querySelector('.northen');
...
const globeRotate = () => {
    if (currentSlide === 0) {
        let globeTop = 0;
        const style = getComputedStyle(globe);
        let int = setInterval(() => {
            let top = style.backgroundPositionY;
            top = top.slice(0, -2);
            globeTop = globeTop - 0.5;
            if (globeTop >= -1100) {
                globe.style.backgroundPositionY = globeTop + 'px';
            }
        }, 80)
    }
}
